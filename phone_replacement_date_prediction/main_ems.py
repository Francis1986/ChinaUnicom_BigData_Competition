...
if Train_On:
    ...
    for train_index,test_index in kf.split(np.arange(0,len(fea_train))):
        ...
        model_xgb = train_xgb(x_train,y_train_gap,x_test,y_test_gap)
        model_lgb.save_model('./model/model_lgb_{}.csv'.format(i))#保存lgb模型
        model_xgb.save_model('./model/model_xgb_{}.csv'.format(i))#保存xgb模型
        print ('模型保存完成！')
        #保存特征重要性
        ...
        #线下预测
        y_pred_gap_lgb = model_lgb.predict(x_test,num_iteration=model_lgb.best_iteration)
        y_pred_gap_xgb = model_xgb.predict(xgb.DMatrix(x_test.values))
        y_pred_gap = (y_pred_gap_lgb+y_pred_gap_xgb)/2
...
if Submit_On:
    ...
    #获取模型列表
    model_list_lgb = glob('./model/model_lgb*')
    model_list_xgb = glob('./model/model_xgb*')
    print ('lightGBM模型列表:',model_list_lgb)
    print ('XGBoost模型列表:',model_list_xgb)
    pred_gap_col_list_lgb=[]
    pred_gap_col_list_xgb=[]
    for i,model in enumerate(model_list_lgb):
        ...
        pred_gap_col_list_lgb.append('y_pred_gap_lgb_{}'.format(i))
    for i,model in enumerate(model_list_xgb):
        model_xgb = xgb.Booster(model_file=model)
        y_pred_gap = model_xgb.predict(xgb.DMatrix(test.values))
        y_pred_df['y_pred_gap_xgb_{}'.format(i)] = y_pred_gap
        pred_gap_col_list_xgb.append('y_pred_gap_xgb_{}'.format(i))
    print ('测试集预测完成，开始保存结果... ...')
    y_pred_df['y_pred_gap_f'] = np.round(y_pred_df[pred_gap_col_list_lgb+pred_gap_col_list_xgb].mean(axis=1))
    ...
    result = y_pred_df[['USER_ID','pred_date_sub']].to_csv('./result/submit_ems.csv',index=None,header=None)
    print('结果保存完毕！')