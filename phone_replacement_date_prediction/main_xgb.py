...
data = data_load()
#特征生成
fea_train,fea_test = fea_gen(data=data,one_hot_col=cate_col,uptime_wash_on=uptime_wash_on,one_hot_on=True)
print (fea_train.columns)
...
if Train_On:
   ...
   # train[cate_col] = train[cate_col].astype('category')
    ...
    for train_index,test_index in kf.split(np.arange(0,len(fea_train))):
        ...
        model_xgb.save_model('./model/model_xgb_{}.csv'.format(i))#保存模型
        print ('模型保存完成！')
        #保存特征重要性
        ...
        #线下预测
        y_pred_gap = model_xgb.predict(xgb.DMatrix(x_test.values))
...
if Submit_On:
    ...
    #获取模型列表
    model_list = glob('./model/model_xgb*')
    print (model_list)
    pred_gap_col_list=[]
    ...
    result = y_pred_df[['USER_ID','pred_date_sub']].to_csv('./result/submit_xgb.csv',index=None,header=None)
    print('结果保存完毕！')